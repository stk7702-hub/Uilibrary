...
export type Window = {
	Name: string,
	Keybind: string | Enum.KeyCode | {string | Enum.KeyCode},
	Scale: UDim2,
	Expire: string,
	Theme: {[string]: Color3}?,
	SidebarWidth: number?,
...
Fatality.ThemeDefaults = {
	...
	-- Новые ключи для детальной настройки:
	SliderAccent = Color3.fromRGB(255, 106, 133), -- Цвет полоски слайдера
	ToggleAccent = Color3.fromRGB(255, 106, 133), -- Цвет включенного чекбокса
	TabSelected = Color3.fromRGB(255, 106, 133),  -- Цвет активного таба
	TabUnselected = Color3.fromRGB(255, 255, 255),-- Цвет неактивного таба
	ProfileStroke = Color3.fromRGB(255, 106, 133),-- Обводка аватарки
...
function Fatality:NewTheme(overrides)
	local t = table.clone(Fatality.ThemeDefaults)
	if overrides then deepMerge(t, overrides) end
	return t
end

-- Подписка UI-объекта на токен темы (live update)
function Fatality:BindTheme(Fatal, inst: Instance, prop: string, token: string)
	if not Fatal or not inst then return end
	Fatal.__themeBinds = Fatal.__themeBinds or {}
	Fatal.__themeBinds[inst] = Fatal.__themeBinds[inst] or {}
	Fatal.__themeBinds[inst][prop] = token
	-- установить сразу
	if Fatal.Theme and Fatal.Theme[token] ~= nil then
		pcall(function() inst[prop] = Fatal.Theme[token] end)
	end
	-- обновлять на смене темы
	if not Fatal.__themeConn then
		Fatal.__themeConn = Fatal.ThemeChanged.Event:Connect(function()
			for obj, map in pairs(Fatal.__themeBinds) do
				if obj and obj.Parent then
					for p, tk in pairs(map) do
						local val = Fatal.Theme[tk]
						if val ~= nil then
							pcall(function() obj[p] = val end)
						end
					end
				end
			end
...
function Fatality:ScrollSignal(Scroll: ScrollingFrame,UIListLayout: UIListLayout,Type:string)
	if Type == 'X' then
		UIListLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
			...
			local paddingX = (padding and (padding.PaddingLeft.Offset + padding.PaddingRight.Offset)) or 0
			Scroll.CanvasSize = UDim2.fromOffset(UIListLayout.AbsoluteContentSize.X + paddingX, 0)
		end)
	else
		UIListLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
			local padding = Scroll:FindFirstChildOfClass("UIPadding")
			local paddingY = (padding and (padding.PaddingTop.Offset + padding.PaddingBottom.Offset)) or 0
			Scroll.CanvasSize = UDim2.fromOffset(0, UIListLayout.AbsoluteContentSize.Y + paddingY)
		end)
	end;
end;
...
function Fatality:CreateElements(Parent : Frame , ZIndex : number , Event : BindableEvent,SearchAPI: {Path: string,Memory : (name:string) -> any}) : Elements
	...
	function elements:AddToggle(Config: Toggle)
		...
		Toggle_Name.AnchorPoint = Vector2.new(0, 0)
		...
		RightContainer.Parent = Toggle
		RightContainer.AnchorPoint = Vector2.new(1, 0)
		RightContainer.BackgroundTransparency = 1
		RightContainer.Position = UDim2.new(1, 0, 0, 0)
		RightContainer.Size = UDim2.new(0, 40, 0, 17)

		Toggle_Name.Size = UDim2.new(1, -45, 0, 17)
		...
		ValueFrame.BackgroundColor3 = Color3.fromRGB(35, 35, 35)  -- ИСПРАВЛЕНО: более светлый цвет
		ValueFrame.BorderColor3 = Color3.fromRGB(0, 0, 0)
		ValueFrame.BorderSizePixel = 0
		ValueFrame.Position = UDim2.new(1, -3, 0, 0)
		ValueFrame.Size = UDim2.new(0, 15, 0, 15)
		ValueFrame.ZIndex = ZIndex + 2
		ValueFrame.BackgroundTransparency = 0;  -- ИСПРАВЛЕНО: убрана прозрачность

		UICorner.CornerRadius = UDim.new(0, 2)
		UICorner.Parent = ValueFrame

		-- ДОБАВЛЕНО: UIStroke для лучшей видимости
		local ValueFrameStroke = Instance.new("UIStroke")
		ValueFrameStroke.Color = Color3.fromRGB(60, 60, 60)
		ValueFrameStroke.Thickness = 1
		ValueFrameStroke.Parent = ValueFrame
		...
		local toggleImg = function(value)
			if value then
				Fatality:CreateAnimation(ValueIcon,0.45,{
					ImageTransparency = 0,
					ImageColor3 = Fatal.Theme.ToggleAccent, -- ИСПОЛЬЗУЕМ ToggleAccent
		...
		local Respons = Fatality:CreateResponse({
			...
			SetVisible = function(bool)
				if Toggle then Toggle.Visible = bool end
			end,
	...
	function elements:AddSlider(Config: Slider)
		...
		RightContainer.BackgroundTransparency = 1
		RightContainer.Position = UDim2.new(1, 0, 0, 0)
		RightContainer.Size = UDim2.new(0, 110, 0, 17)

		Slider_Name.Name = Fatality:RandomString()
		Slider_Name.Parent = Slider
		Slider_Name.AnchorPoint = Vector2.new(0, 0)
		...
		Slider_Name.Position = UDim2.new(0, 0, 0, 0)
		Slider_Name.Size = UDim2.new(1, -115, 0, 17)
		Slider_Name.AutomaticSize = Enum.AutomaticSize.Y
		...
		ValueFrame.BorderColor3 = Color3.fromRGB(0, 0, 0)
		ValueFrame.BorderSizePixel = 0
		ValueFrame.Position = UDim2.new(1, -3, 0, 2)
		ValueFrame.Size = UDim2.new(0, 85, 0, 12)
		ValueFrame.ZIndex = ZIndex + 2
		...
		OptionButton.BorderColor3 = Color3.fromRGB(0, 0, 0)
		OptionButton.BorderSizePixel = 0
		OptionButton.Position = UDim2.new(0, 0, 0, 2)
		OptionButton.Size = UDim2.new(0, 13, 0, 13)
		OptionButton.Image = "http://www.roblox.com/asset/?id=14007344336"
		...
		boxli.BackgroundColor3 = Fatal.Theme.SliderAccent -- ИСПОЛЬЗУЕМ SliderAccent
		...
		local function update(Input)
			...
			TweenService:Create(boxli , TweenInfo.new(0.2),{
				...
				BackgroundColor3 = Fatal.Theme.SliderAccent -- Гарантируем цвет
			}):Play();
		...
		-- Обновление цвета слайдера при изменении темы
		if Fatal and Fatal.ThemeChanged then
			Fatal.ThemeChanged.Event:Connect(function()
				boxli.BackgroundColor3 = Fatal.Theme.SliderAccent
			end)
		end
		...
		local Respons = Fatality:CreateResponse({
			...
			SetVisible = function(bool)
				if Slider then Slider.Visible = bool end
			end,
	...
	function elements:AddColorPicker(Config: ColorPicker)
		...
		local RightContainer = Instance.new("Frame")
		RightContainer.Name = Fatality:RandomString()
		RightContainer.Parent = ColorPicker
		RightContainer.AnchorPoint = Vector2.new(1, 0)
		RightContainer.BackgroundTransparency = 1
		RightContainer.Position = UDim2.new(1, 0, 0, 0)
		RightContainer.Size = UDim2.new(0, 60, 0, 17)

		ColorPicker_Name.Name = Fatality:RandomString()
		ColorPicker_Name.Parent = ColorPicker
		ColorPicker_Name.AnchorPoint = Vector2.new(0, 0)
		...
		ColorPicker_Name.Size = UDim2.new(1, -65, 0, 17)
		ColorPicker_Name.AutomaticSize = Enum.AutomaticSize.Y
		...
		ColorPicker_Name.TextYAlignment = Enum.TextYAlignment.Top
		ColorPicker_Name.TextWrapped = true
		ColorPicker_Name.ZIndex = ZIndex + 2
		...
		ValueFrame.BorderSizePixel = 0
		ValueFrame.Position = UDim2.new(1, -3, 0, 2)
		ValueFrame.Size = UDim2.new(0, 35, 0, 12)
	...
	function elements:AddDropdown(Config: Dropdown)
		...
		local RightContainer = Instance.new("Frame")
		RightContainer.Name = Fatality:RandomString()
		RightContainer.Parent = Dropdown
		RightContainer.AnchorPoint = Vector2.new(1, 0)
		RightContainer.BackgroundTransparency = 1
		RightContainer.Position = UDim2.new(1, 0, 0, 0)
		...
		Dropdown_Name.AnchorPoint = Vector2.new(0, 0)
		...
		Dropdown_Name.Size = UDim2.new(1, -105, 0, 17)
		Dropdown_Name.AutomaticSize = Enum.AutomaticSize.Y
		...
		Dropdown_Name.TextYAlignment = Enum.TextYAlignment.Top
		Dropdown_Name.TextWrapped = true
		...
		ValueFrame.BorderSizePixel = 0
		ValueFrame.Position = UDim2.new(1, -3, 0, 0)
		ValueFrame.Size = UDim2.new(0, 75, 0, 16)
		...
		local Respons = Fatality:CreateResponse({
			...
			SetVisible = function(bool)
				if Dropdown then Dropdown.Visible = bool end
			end,
...
function Fatality:CreateConfigWindow(Root: ScreenGui , Fatal , Button: ImageButton)
	...
	res = Fatality:CreateResponse({
		...
		SaveConfig = function(name,content)
			...
			if name == "" then
				warn("[Fatality Config] empty config name")
				return
			end
		...
		LoadConfig = function(name)
			...
			local okDecode, decoded = pcall(function()
				return game:GetService('HttpService'):JSONDecode(rawConfig)
			end)
		...
		Init = function(Name: string,Folder: string)
			Folder = res:SanitizeToken(Folder or "Fatality")
			Name = res:SanitizeToken(Name or "Fatality")
			...
			AddConfigButton.MouseButton1Click:Connect(function()
				local configName = res:NormalizeConfigName(TextBox.Text);
...
function Fatality.new(Window: Window)
	...
	local bottomHeight = tonumber(Window.BottomHeight) or 25;
	local tabHeight = tonumber(Window.TabHeight) or 40;
	local headerHeight = tonumber(Window.HeaderHeight) or 40;
	...
	Fatal.HeaderHeight = headerHeight
	Fatal.BottomHeight = bottomHeight
	
	function Fatal:SetTheme(partialTheme: table)
		if typeof(partialTheme) ~= "table" then return end
		deepMerge(self.Theme, partialTheme)
		self.ThemeChanged:Fire(self.Theme)
	end
	function Fatal:GetTheme()
		return self.Theme
	end
	...
	local ToggleUI = function(bool)
		...
		if bool then
			...
			FatalFrame.BackgroundTransparency = 0
			
			Header.BackgroundTransparency = 0
			HeaderLine.BackgroundTransparency = 0
			Bottom.BackgroundTransparency = 0
			HeaderLine_2.BackgroundTransparency = 0
			
			HeaderLineShadow.BackgroundTransparency = 0.5
			HeaderLineShadow_2.BackgroundTransparency = 0.5
			...
			HeaderText.TextTransparency = 0

			UserIcon.ImageTransparency = 0
			UserIcon.Position = UDim2.new(0, 10, 0.5, 2)

			User_name.TextTransparency = 0
			User_name.Position = UDim2.new(0, 50, 0.5, -6)

			expire_days.TextTransparency = 0
			expire_days.Position = UDim2.new(0, 50, 0.5, 8)
			...
			-- МГНОВЕННОЕ СКРЫТИЕ (без анимаций)
			table.clear(Fatality.DragBlacklist);

			-- Скрываем контент меню
			for i, v in next, Fatal.Menus do
				if v.Bindable then
					v.Bindable:SetAttribute('V', false)
					v.Bindable:Fire(false)
				end
			...
			-- Главный фрейм в последнюю очередь
	...
	-- Сохраняем ссылку на Fatal в глобальной таблице для доступа из элементов
	...
	Fatality:BindTheme(Fatal, FatalFrame, "BackgroundColor3", "Background")
	...
	DropShadow.SliceCenter = Rect.new(49, 49, 450, 450)
	DropShadow.Rotation = 0.001
	Fatality:BindTheme(Fatal, DropShadow, "ImageColor3", "Shadow")
	...
	Header.ZIndex = 2
	Fatality:BindTheme(Fatal, Header, "BackgroundColor3", "Header")
	...
	HeaderLine.ZIndex = 3
	Fatality:BindTheme(Fatal, HeaderLine, "BackgroundColor3", "Stroke")
	...
	HeaderText.AnchorPoint = Vector2.new(0, 0.5) -- Оставляем якорь слева
	HeaderText.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
	HeaderText.BackgroundTransparency = 1.000
	HeaderText.BorderColor3 = Color3.fromRGB(0, 0, 0)
	HeaderText.BorderSizePixel = 0
	-- Позиция 0,0 - начало хедера.
	HeaderText.Position = UDim2.new(0, 0, 0.5, 0)
	-- Ширина равна ширине сайдбара
	HeaderText.Size = UDim2.new(0, sidebarWidth, 0.699999988, 0)
	HeaderText.ZIndex = 4
	...
	Sidebar.Name = Fatality:RandomString()
	Sidebar.Parent = FatalFrame
	Sidebar.BackgroundTransparency = 0
	Sidebar.BorderSizePixel = 0
	Sidebar.Position = UDim2.new(0, 0, 0, 0)
	Sidebar.Size = UDim2.new(0, sidebarWidth, 1, 0)
	Sidebar.ZIndex = 3
	Fatality:BindTheme(Fatal, Sidebar, "BackgroundColor3", "Header")

	local SidebarStroke = Instance.new("UIStroke")
	SidebarStroke.Parent = Sidebar
	SidebarStroke.Thickness = 1
	Fatality:BindTheme(Fatal, SidebarStroke, "Color", "Stroke")

	-- Tabs container (bottom-up)
	local TabsContainer = Instance.new("Frame")
	TabsContainer.Name = Fatality:RandomString()
	TabsContainer.Parent = Sidebar
	TabsContainer.BackgroundTransparency = 1
	TabsContainer.BorderSizePixel = 0
	TabsContainer.Position = UDim2.new(0, 0, 0, headerHeight + 5) -- Отступ сверху для Header
	TabsContainer.Size = UDim2.new(1, 0, 1, -50 - headerHeight - 5) -- Уменьшено с 72 до 50 под новый профиль

	local TabsScroll = Instance.new("ScrollingFrame")
	TabsScroll.Name = Fatality:RandomString()
	...
	TabsPadding.PaddingTop = UDim.new(0, 10) -- Увеличенный отступ сверху
	...
	UIListLayout.SortOrder = Enum.SortOrder.LayoutOrder
	UIListLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
	UIListLayout.VerticalAlignment = Enum.VerticalAlignment.Top -- Сверху вниз
	UIListLayout.Padding = UDim.new(0, 6)

	Fatality:ScrollSignal(TabsScroll, UIListLayout, "Y")

	-- Profile container (bottom of sidebar) - HORIZONTAL LAYOUT
	UserProfle.Name = Fatality:RandomString()
	UserProfle.Parent = Sidebar
	UserProfle.BackgroundTransparency = 1
	UserProfle.BorderSizePixel = 0
	UserProfle.AnchorPoint = Vector2.new(0, 1)
	UserProfle.Position = UDim2.new(0, 0, 1, 0)
	UserProfle.Size = UDim2.new(1, 0, 0, 50) -- Уменьшенная высота для горизонтального layout
	UserProfle.ZIndex = 4

	-- Добавить разделительную линию сверху профиля
	local ProfileSeparator = Instance.new("Frame")
	ProfileSeparator.Name = Fatality:RandomString()
	ProfileSeparator.Parent = UserProfle
	ProfileSeparator.BackgroundColor3 = Color3.fromRGB(29, 29, 29)
	ProfileSeparator.BorderSizePixel = 0
	ProfileSeparator.Position = UDim2.new(0, 8, 0, 0)
	ProfileSeparator.Size = UDim2.new(1, -16, 0, 1)
	ProfileSeparator.ZIndex = 4
	Fatality:BindTheme(Fatal, ProfileSeparator, "BackgroundColor3", "Stroke")

	-- Assign tbc to TabsScroll for compatibility
	local tbc = TabsScroll

	-- Avatar - LEFT ALIGNED
	UserIcon.Name = Fatality:RandomString()
	UserIcon.Parent = UserProfle
	UserIcon.AnchorPoint = Vector2.new(0, 0.5) -- Якорь слева по центру
	...
	UserIcon.Position = UDim2.new(0, 10, 0.5, 2) -- Слева с отступом, по центру вертикально
	UserIcon.Size = UDim2.new(0, 32, 0, 32) -- Размер аватарки
	UserIcon.ZIndex = 5
	...
	-- === ПРИВЯЗКА К PROFILESTROKE ===
	Fatality:BindTheme(Fatal, UIStroke, "Color", "ProfileStroke")

	-- Username - RIGHT OF AVATAR, TOP
	User_name.Name = Fatality:RandomString()
	User_name.Parent = UserProfle
	User_name.AnchorPoint = Vector2.new(0, 0.5) -- Якорь слева
	...
	User_name.Position = UDim2.new(0, 50, 0.5, -6) -- Справа от аватарки (10 + 32 + 8 = 50), выше центра
	User_name.Size = UDim2.new(1, -60, 0, 14)
	User_name.ZIndex = 4
	User_name.Font = Enum.Font.GothamMedium
	User_name.Text = Client.DisplayName
	User_name.TextColor3 = Color3.fromRGB(255, 255, 255)
	User_name.TextSize = 12.000
	User_name.TextStrokeTransparency = 0.700
	User_name.TextXAlignment = Enum.TextXAlignment.Left -- Выравнивание по левому краю
	User_name.TextTruncate = Enum.TextTruncate.AtEnd -- Обрезка длинных ников
	Fatality:BindTheme(Fatal, User_name, "TextColor3", "UsernameText")

	-- Expire date - RIGHT OF AVATAR, BOTTOM (under username)
	expire_days.Name = Fatality:RandomString()
	expire_days.Parent = UserProfle
	expire_days.AnchorPoint = Vector2.new(0, 0.5) -- Якорь слева
	...
	expire_days.Position = UDim2.new(0, 50, 0.5, 8) -- Справа от аватарки, ниже центра
	...
	MenuFrame.Position = UDim2.new(0, sidebarWidth, 0, headerHeight) -- Начинается ПОД header
	MenuFrame.Size = UDim2.new(1, -sidebarWidth, 1, -(headerHeight + bottomHeight)) -- Между header и bottom
	MenuFrame.ClipsDescendants = true -- ВАЖНО: обрезает контент
	Fatality:BindTheme(Fatal, MenuFrame, "BackgroundColor3", "Background")
	...
	Bottom.Position = UDim2.new(0, sidebarWidth, 1, 0)
	Bottom.Size = UDim2.new(1, -sidebarWidth, 0, bottomHeight)
	Bottom.ZIndex = 2
	Fatality:BindTheme(Fatal, Bottom, "BackgroundColor3", "Header")
	...
	Fatality:BindTheme(Fatal, HeaderLine_2, "BackgroundColor3", "Stroke")
	...
	function Fatal:SetToggleBinds(binds)
		...
		for _,b in ipairs(binds) do
			local nb = normalizeBind(b)
			if nb then table.insert(self.ToggleBinds, nb) end
		end
	end
	function Fatal:SetToggleKeybind(bind)
		self:SetToggleBinds({ bind })
	end

	-- init from Window.Keybind
	Fatal:SetToggleBinds(Window.Keybind or Enum.KeyCode.Insert)

	UserInputService.InputBegan:Connect(function(input, istyping)
		if istyping then return end
		for _,b in ipairs(Fatal.ToggleBinds) do
			if inputMatchesBind(input, b) then
	...
	function Fatal:AddMenu(Menu : Menu)
		...
		-- Счетчик для чередования секций (Лево -> Право -> Лево...)
		...
		MenuButton.Size = UDim2.new(1, -16, 0, self.TabHeight or 40)
		MenuButton.ZIndex = 5
		...
		menu_name.Position = UDim2.new(0, 36, 0.5, 0)
		menu_name.Size = UDim2.new(1, -36, 1, 0)
		menu_name.ZIndex = 5
		...
		-- НАЧАЛЬНАЯ ИНИЦИАЛИЗАЦИЯ ЦВЕТОВ И ПРОЗРАЧНОСТИ
		Icon.ImageColor3 = Fatal.Theme.TabUnselected
		Icon.ImageTransparency = 0.5
		menu_name.TextColor3 = Fatal.Theme.TabUnselected
		menu_name.TextTransparency = 0.5
		MenuButton.BackgroundTransparency = 1

		local MenuLiber = Instance.new("Frame")
		local Left = Instance.new("ScrollingFrame")
		local UIListLayout = Instance.new("UIListLayout")
		-- Center убран, так как переходим на 2 колонки
		...
		-- === НАСТРОЙКА ЛЕВОЙ КОЛОНКИ (50% ширины) ===
		...
		Left.Position = UDim2.new(0, 6, 0, 5) -- Отступ 6px слева
		Left.Size = UDim2.new(0.5, -9, 1, -10) -- Половина ширины минус отступы
		Left.ScrollBarThickness = 2
		Left.ScrollBarImageColor3 = Color3.fromRGB(60, 60, 60)
		...
		-- === НАСТРОЙКА ПРАВОЙ КОЛОНКИ (50% ширины) ===
		...
		Right.Position = UDim2.new(1, -6, 0, 5) -- Отступ 6px справа
		Right.Size = UDim2.new(0.5, -9, 1, -10) -- Половина ширины минус отступы
		Right.ScrollBarThickness = 2
		Right.ScrollBarImageColor3 = Color3.fromRGB(60, 60, 60)
		...
		Fatal.Signal.Event:Connect(function(Bool)
			if Bool then
				...
				-- Анимация появления (приезжают с краев)
				...
				-- Анимация скрытия (уезжают вниз)
				local HIDE_DURATION = 0.2 
				
				Fatality:CreateAnimation(Left, HIDE_DURATION, {
					Position = UDim2.new(0, 6, 1, -10)
				})

				Fatality:CreateAnimation(Right, HIDE_DURATION, {
					Position = UDim2.new(1, -6, 1, -10)
				})

				Fatality:CreateAnimation(MenuButton, HIDE_DURATION, {
					BackgroundTransparency = 1
				})

				Fatality:CreateAnimation(UIStroke, HIDE_DURATION, {
					Transparency = 1
				})

				Fatality:CreateAnimation(Icon, HIDE_DURATION, {
					ImageTransparency = 1
				})

				Fatality:CreateAnimation(menu_name, HIDE_DURATION, {
					TextStrokeTransparency = 1,
					TextTransparency = 1
				})
			end;
		...
		local ValueSelect = function(val)
			if val then
				...
				Fatality:CreateAnimation(Icon,0.5,{
					ImageTransparency = 0.15,
					ImageColor3 = Fatal.Theme.TabSelected -- ИСПОЛЬЗУЕМ ТЕМУ
				});

				Fatality:CreateAnimation(menu_name,0.5,{
					TextTransparency = 0.15,
					TextColor3 = Fatal.Theme.TabSelected -- ОПЦИОНАЛЬНО: красим текст тоже
				});
				...
				Fatality:CreateAnimation(Icon,0.5,{
					ImageTransparency = 0.5,
					ImageColor3 = Fatal.Theme.TabUnselected -- ИСПОЛЬЗУЕМ ТЕМУ
				});
				...
				Fatality:CreateAnimation(menu_name,0.5,{
					TextTransparency = 0.5,
					TextColor3 = Fatal.Theme.TabUnselected -- ИСПОЛЬЗУЕМ ТЕМУ
				});
			end;
		end;

		-- Устанавливаем LayoutOrder для правильного порядка в вертикальном списке
		-- При VerticalAlignment.Top: меньший LayoutOrder = выше, больший = ниже
		MenuButton.LayoutOrder = #Fatal.Menus + 1
		...
		Fatality:CreateHover(MenuButton, function(bool)
			-- Только если этот таб НЕ выбран
			if Fatal.MenuSelected.Root ~= MenuLiber then
				if bool then
					-- При наведении - белый цвет, менее прозрачный
					Fatality:CreateAnimation(Icon, 0.3, {
						ImageTransparency = 0.2,
						ImageColor3 = Color3.fromRGB(255, 255, 255)
					})
					Fatality:CreateAnimation(menu_name, 0.3, {
						TextTransparency = 0.2,
						TextColor3 = Color3.fromRGB(255, 255, 255)
					})
				else
					-- При уходе курсора - возврат к НЕАКТИВНОМУ состоянию из темы
					Fatality:CreateAnimation(Icon, 0.3, {
						ImageTransparency = 0.5,
						ImageColor3 = Fatal.Theme.TabUnselected  -- ИСПРАВЛЕНО!
					})
					Fatality:CreateAnimation(menu_name, 0.3, {
						TextTransparency = 0.5,
						TextColor3 = Fatal.Theme.TabUnselected  -- ИСПРАВЛЕНО!
					})
				end
			end
		end)
		...
		function MenuLib:AddPreview(Config: Preview)
			...
			-- Если запрошен center, кидаем в right или left по желанию, 
			-- но лучше просто использовать Left/Right
			local targetParent = (string.lower(Config.Position) == 'left' and Left) or Right;
			Preview.Parent = targetParent;
		...
		function MenuLib:AddListBox(Config : Listbox)
			...
			-- Аналогично Preview, меняем логику выбора родителя
			local targetParent = (string.lower(Config.Position) == 'left' and Left) or Right;
			ListBox.Name = Fatality:RandomString()
			ListBox.Parent = targetParent;
		...
		function MenuLib:AddSection(Config : Section)
			...
			Config.Height = Config.Height or 0;
			-- НОВЫЕ ПАРАМЕТРЫ:
			Config.Side = Config.Side or "auto";  -- "left", "right", "auto"
			Config.ShowTitle = (Config.ShowTitle == nil) and true or Config.ShowTitle;  -- По умолчанию true
			...
			-- НОВАЯ ЛОГИКА ВЫБОРА КОЛОНКИ:
			local targetParent
			if Config.Side == "left" then
				targetParent = Left
			elseif Config.Side == "right" then
				targetParent = Right
			else
				-- auto - чередование
				if SectionCounter % 2 == 0 then
					targetParent = Left
				else
					targetParent = Right
				end
			...
			-- Высота заголовка зависит от ShowTitle
			local titleHeight = Config.ShowTitle and 22 or 0
			...
			Section.ZIndex = 10
			Fatality:BindTheme(Fatal, Section, "BackgroundColor3", "Panel")
			...
			SectionName.Position = UDim2.new(0, 0, 0, 2)
			SectionName.Size = UDim2.new(1, 0, 0, 18)
			...
			SectionName.TextXAlignment = Enum.TextXAlignment.Center  -- ПО ЦЕНТРУ!
			SectionName.ZIndex = 12
			SectionName.Visible = Config.ShowTitle  -- Управление видимостью
			Fatality:BindTheme(Fatal, SectionName, "TextColor3", "Text")
			...
			Elements.BorderColor3 = Color3.fromRGB(0, 0, 0)
			Elements.BorderSizePixel = 0
			Elements.Position = UDim2.new(0.5, 0, 0, titleHeight + 2)  -- Отступ под заголовок
			Elements.Size = UDim2.new(1, -6, 0, 0)
			Elements.ClipsDescendants = true
			Elements.ZIndex = 11
			Fatality:BindTheme(Fatal, Elements, "BackgroundColor3", "Field")
			...
			-- Динамическое обновление размера
			UIListLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
				local contentHeight = UIListLayout.AbsoluteContentSize.Y
				local newHeight = math.max(contentHeight + 15, 30) 
				
				Elements.Size = UDim2.new(1, -6, 0, newHeight)
				
				task.defer(function()
					-- titleHeight учитывается в общей высоте секции
					local sectionHeight = titleHeight + 5 + newHeight + 5 + Config.Height
					if not Menu.AutoFill then
						Fatality:CreateAnimation(Section, 0.2, {
							Size = UDim2.new(1, 0, 0, sectionHeight)
						})
					else
						Section.Size = UDim2.new(1, 0, 0, math.max(sectionHeight * 0.8, 60))
					end
				end)
			end)
			...
			SpaceBox.Size = UDim2.new(0, 0, 0, 5)
